...
typedef NS_ENUM(NSUInteger, MPErrorSource) {
    ...
    /** The accessory parameters are invalid or the accessory caused an error */
...
typedef NS_ENUM(NSUInteger, MPErrorType){
    ...
    /** Offline batch couldn't be processed because it's malformed. Will be checked manually */
    MPErrorTypeOfflineBatchMalformedPendingManualReview,
    ...
    MPErrorTypeServerError,
    /** The server returned rubish */
    MPErrorTypeServerInvalidResponse,
    /** Processor on server side is not available */
    MPErrorTypeServerGatewayUnavailable,
    ...
    MPErrorTypeSDKFeatureNotEnabled,
    //39
