 ...
 */
typedef void (^MPAccessoryUpdateStateSuccess)(MPAccessory * _Nonnull accessory);
 ...
 * @param error Error describing why the update failed
 */
typedef void (^MPAccessoryUpdateStateFailure)(MPAccessory * _Nonnull accessory, NSError * _Nonnull error);
...
/**
 * Accept handler for executing an offline tranaction. At this stage the transaction is accepted offline and is ready to be updated with the next batch
 * to be executed by the host.
 * @param transaction The accepted transaction
 * @since 2.14.0
 */
typedef void (^MPTransactionAccept)(MPTransaction * _Nonnull transaction);
 ...
 *
 * @param transaction The captured transaction
 * @since 2.8.0
 */
...
/**
 * Success handler for tip adjust transaction.
 *
 * @param transaction The tip adjust transaction
 * @since 2.22.0
 */
typedef void (^MPTransactionTipAdjustSuccess)(MPTransaction * _Nonnull transaction);

/**
 * Failure handler for tip adjust transaction.
 *
 * @param transactionParameters The tip adjust transaction parameters
 * @param error Describes why the tip adjust transaction failed (if it failed due to an internal error)
 * @since 2.22.0
 */
typedef void (^MMPTransactionTipAdjustFailure)(MPTransactionParameters* _Nonnull transactionParameters, NSError * _Nonnull error);
...
/**
 * Failure handler for a transaction query
 * @param error Error describing why query failed
 * @since 2.6.0
 */
...
/**
 * Returns a factory that must be used to generate the payment parameters when making a transaction.
 * @since 2.0.0
 */
...
/**
 * Execute a transaction that does not require an accessory.
 *
 * @param transaction Transaction which is to be executed.
 * @param accountParameters Transaction parameters for the transaction which is to be executed.
 * @param approval The approved handler called when the transaction was approved.
 * @param decline The declined handler called when the transaction was declined during the execution.
 * @param abort The abort handler called when the transaction was aborted during the execution.
 * @param failure The failure handler called when the transaction failed, have a look at the error for more details
 * @since 2.22.0
 */
- (void)executeTransaction:(nonnull MPTransaction *)transaction accountParameters:(nonnull MPAccountParameters *)accountParameters approval:(nonnull MPTransactionApproval)approval decline:(nonnull MPTransactionDecline)decline abort:(nonnull MPTransactionAbort)abort failure:(nonnull MPTransactionFailure)failure;
...
#pragma mark -
#pragma mark Tip Adjust Transation

/**
 * Adjust tip for a transaction with the given parameters
 * @param transactionParameters The transaction parameters for tip adjust
 * @param success The success handler called when the tip adjust succeed
 * @param failure The failure handler called when the tip adjust failed
 * @throws NSException If the transaction is invalid
 * @since 2.22.0
 */
- (void)tipAdjustTransactionWithParameters:(nonnull MPTransactionParameters *)transactionParameters success:(nonnull MPTransactionTipAdjustSuccess)success failure:(nonnull MMPTransactionTipAdjustFailure)failure;
